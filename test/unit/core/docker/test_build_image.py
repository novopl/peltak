...
@pytest.mark.parametrize('image,expected_cmd', [
    (
        {'name': 'fakeimg'},
        'docker build -t {reg}/{img} -t {reg}/{img}:{ver} .'
    ),
    (
        {'name': 'fakeimg', 'file': '/Dockerfile.fake'},
        'docker build -t {reg}/{img} -t {reg}/{img}:{ver} -f /Dockerfile.fake .'
    ),
])
...
def test_build_image_generates_proper_shell_command(p_run, image, expected_cmd):
